...
static err_t low_level_output(struct netif *netif, struct pbuf *p) {
    ...
    for (q = p; q != NULL; q = q->next) {
        if (i >= ETH_TX_DESC_CNT) {
            DIAG(ETH_DIAG "low level output: Txbuffer too small");
            return ERR_IF;
        }
    ...
    if (status == HAL_OK) {
...
#if 0
        DIAG(ETH_DIAG "low level input: RxArraySection %p", Rx_Buff);
        DIAG(ETH_DIAG "low level input: RxBuff->buffer %p", RxBuff->buffer);
        DIAG(ETH_DIAG "low level input: retrieving frame length %lu", framelength);
        DIAG(ETH_DIAG "low level input: max frame length %d", ETH_RX_BUFFER_SIZE_ALIGNED);
#endif
        ...
        if (custom_pbuf != NULL) {
            ...
            if (!p)
                ...
                DIAG(ETH_DIAG "low level input: custom_pbuf %p", p);
                DIAG(ETH_DIAG "low level input: custom_pbuf payload %p", p->payload);
            }
#endif
        } else {
