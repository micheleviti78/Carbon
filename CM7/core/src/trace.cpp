...
static err_t sendEvent(TraceTaskSwitchedInEvent &event, struct netconn *conn);

static err_t sendEvent(TraceTaskSwitchedOutEvent &event, struct netconn *conn);
...
err_t sendEvent(TraceMallocEvent &event, struct netconn *conn) {
    ...
    return netconn_write(conn, &event, sizeof(TraceMallocEvent), NETCONN_COPY);
}
...
err_t sendEvent(TracePerfCntEvent &event, struct netconn *conn) {
    ...
    return netconn_write(conn, &event, sizeof(TracePerfCntEvent), NETCONN_COPY);
}
