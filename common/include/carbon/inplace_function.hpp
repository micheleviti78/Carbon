...
#define SG14_INPLACE_FUNCTION_THROW(x) assert(0)
#endif
...
public:
...
#if __cplusplus >= 201703L
        static_assert(
            std::is_invocable_r<R, C, Args...>::value,
    ...
    operator inplace_function<R(Args...), Cap, Align>() const & {
        static_assert(
            inplace_function_detail::is_valid_inplace_dst<Cap, Align, Capacity,
                                                          Alignment>::value,
    ...
    void swap(inplace_function &other) noexcept {
        ...
        storage_t tmp;
        vtable_ptr_->relocate_ptr(std::addressof(tmp),
