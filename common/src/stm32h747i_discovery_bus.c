 ...
 *          with  external devices available on STM32H747I-DISCO board (MB1248)
...
/** @defgroup STM32H747I_DISCO_BUS_Private_FunctionPrototypes Private
 * FunctionPrototypes
 * @{
 */
    ...
    if (I2c4InitCounter == 0U) {
        ...
        if (HAL_I2C_GetState(&hbus_i2c4) == HAL_I2C_STATE_RESET) {
            ...
            if (BspI2cSemaphore == NULL) {
                ...
                BspI2cSemaphore =
                    osSemaphoreCreate(osSemaphore(BSP_I2C_SEM), 1);
            }
#endif
...
#endif
            if (MX_I2C4_Init(&hbus_i2c4, I2C_GetTiming(HAL_RCC_GetPCLK2Freq(),
                                                       BUS_I2C4_FREQUENCY)) !=
                HAL_OK) {
        ...
        for (scll = 0; scll < I2C_SCLL_MAX; scll++) {
            ...
            if ((tscl_l > I2C_Charac[I2C_speed].lscl_min) &&
                ...
                for (sclh = 0; sclh < I2C_SCLH_MAX; sclh++) {
                    /* tHIGH(min) <= tAF(min) + tDNF + 2 x tI2CCLK + [(SCLH+1) x
                     * tPRESC] */
