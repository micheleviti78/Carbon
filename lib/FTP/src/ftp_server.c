...
//
__weak uint8_t ftp_eth_is_connected(void) { return 1; }
...
static void ftp_send(ftp_data_t *ftp, const char *fmt, ...) {
    ...
    vsnprintf_(send_buffer, FTP_BUF_SIZE, fmt, args);

    // Close vaarg list
    va_end(args);

    // send to endpoint
    if (netconn_write(ftp->ctrlconn, send_buffer, strlen(send_buffer),
                      NETCONN_COPY) != ERR_OK)
...
static char *data_time_to_str(char *str, uint16_t date, uint16_t time) {
    snprintf_(str, 25, "%04d%02d%02d%02d%02d%02d",
              ((date & 0xFE00) >> 9) + 1980, (date & 0x01E0) >> 5,
              date & 0x001F, (time & 0xF800) >> 11, (time & 0x07E0) >> 5,
              (time & 0x001F) << 1);
    return str;
}

// Calculate date and time from first parameter sent by MDTM command
// (YYYYMMDDHHMMSS)
//
// parameters:
...
static int8_t date_time_get(char *parameters, uint16_t *pdate,
                            uint16_t *ptime) {
...
deletebuf:
    ...
    // feedback
    DEBUG_PRINT(ftp, "Incomming: %s %s", ftp->command, ftp->parameters);
...
static int pasv_con_open(ftp_data_t *ftp) {
    ...
    if (err != ERR_OK) {
        DEBUG_PRINT(ftp, "Error in opening listening con, bind failed %d", err);
        return -1;
    }
...
static int data_con_open(ftp_data_t *ftp) {
    ...
    // we are in active mode
        ...
        // did connection fail?
        if (netconn_connect(ftp->dataconn, &ftp->ipclient, ftp->data_port) !=
            ERR_OK) {
...
static void ftp_cmd_cwd(ftp_data_t *ftp) {
    ...
    if (strcmp(ftp->path, "/") != 0 &&
        ftps_f_stat(ftp->path, &ftp->finfo) != FR_OK) {
...
static void ftp_cmd_pasv(ftp_data_t *ftp) {
    ...
    // set data port
    ftp->data_port = FTP_DATA_PORT + ftp->data_port_incremented +
                     (ftp->ftp_con_num * PORT_INCREMENT_OFFSET);
    ...
    if (pasv_con_open(ftp) == 0) {
        ...
        ftp_send(ftp, "227 Entering Passive Mode (%d,%d,%d,%d,%d,%d).\r\n",
                 ...
                 (ftp->ipserver.addr >> 16) & 0xFF,
                 (ftp->ipserver.addr >> 24) & 0xFF, ftp->data_port >> 8,
                 ftp->data_port & 255);

        // feedback
        DEBUG_PRINT(ftp, "Data port set to %u", ftp->data_port);
...
static void ftp_cmd_port(ftp_data_t *ftp) {
    ...
    // feedback
    DEBUG_PRINT(ftp, "Data IP set to %u:%u:%u:%u", ip[0], ip[1], ip[2], ip[3]);
    DEBUG_PRINT(ftp, "Data port set to %u", ftp->data_port);
...
static void ftp_cmd_list(ftp_data_t *ftp) {
    ...
    while (ftps_f_readdir(&dir, &ftp->finfo) == FR_OK) {
        ...
        if (strcmp(ftp->command, "LIST"))
            snprintf_(dir_name_buf, FTP_BUF_SIZE, "%s\r\n",
        ...
        // just a file
            ...
            snprintf_(dir_name_buf, FTP_BUF_SIZE, "+r,s%lu,\t%s\r\n",
                      ftp->finfo.fsize,
                      ftp->lfn[0] == 0 ? ftp->finfo.fname : ftp->lfn);

        // write data to endpoint
        netconn_write(ftp->dataconn, dir_name_buf, strlen(dir_name_buf),
                      NETCONN_COPY);
    }
...
static void ftp_cmd_mlsd(ftp_data_t *ftp) {
    ...
    while (ftps_f_readdir(&dir, &ftp->finfo) == FR_OK) {
        ...
        if (ftp->finfo.fdate != 0) {
            char date_str[64];
            snprintf_(
                buf, FTP_BUF_SIZE, "Type=%s;Size=%lu;Modify=%s; %s\r\n",
        ...
        // file has no date
            ...
            snprintf_(buf, FTP_BUF_SIZE, "Type=%s;Size=%lu; %s\r\n",
                      ftp->finfo.fattrib & AM_DIR ? "dir" : "file",
                      ftp->finfo.fsize,
                      ftp->lfn[0] == 0 ? ftp->finfo.fname : ftp->lfn);
        }
...
static void ftp_cmd_retr(ftp_data_t *ftp) {
    ...
    // feedback
    DEBUG_PRINT(ftp, "Sending %s", ftp->parameters);
    ...
    while (1) {
        // read from file ok?
        if (ftps_f_read(&ftp->file, buf, FTP_BUF_SIZE, (UINT *)&bytes_read) !=
            FR_OK) {
        ...
        err_t con_err =
            netconn_write(ftp->dataconn, buf, bytes_read, NETCONN_COPY);
        if (con_err != ERR_OK) {
    ...
    // feedback
    DEBUG_PRINT(ftp, "Sent %d bytes", bytes_transfered);
...
static void ftp_cmd_stor(ftp_data_t *ftp) {
    ...
    // does the path exist?
    if (ftps_f_open(&ftp->file, ftp->path, FA_CREATE_ALWAYS | FA_WRITE) !=
        FR_OK) {
        // go up a level again
        path_up_a_level(ftp->path);
    ...
    // feedback
    DEBUG_PRINT(ftp, "Receiving %s", ftp->parameters);
    ...
    while (1) {
        ...
        while (buflen > 0) {
            ...
            if (offset == FTP_BUF_SIZE) {
                // write data to file
                file_err = ftps_f_write(&ftp->file, buf, FTP_BUF_SIZE,
                                        (UINT *)&bytes_written);
    ...
    if (offset > 0 && file_err == 0) {
        file_err =
            ftps_f_write(&ftp->file, buf, offset, (UINT *)&bytes_written);
    }

    // feedback
    DEBUG_PRINT(ftp, "Received %lu bytes", bytes_transfered);
...
static void ftp_cmd_mkd(ftp_data_t *ftp) {
    ...
    // feedback
    DEBUG_PRINT(ftp, "Creating directory %s", ftp->parameters);
...
void ftp_cmd_rmd(ftp_data_t *ftp) {
    ...
    // feedback
    DEBUG_PRINT(ftp, "Deleting %s", ftp->path);
...
static void ftp_cmd_rnfr(ftp_data_t *ftp) {
    ...
    // feedback
    DEBUG_PRINT(ftp, "Renaming %s", ftp->path_rename);
...
static void ftp_cmd_rnto(ftp_data_t *ftp) {
    ...
    // feedback
    DEBUG_PRINT(ftp, "Renaming %s to %s", ftp->path_rename, ftp->path);
...
static void ftp_cmd_site(ftp_data_t *ftp) {
    ...
    if (!strcmp(ftp->parameters, "FREE")) {
        ...
        ftp_send(ftp, "211 %lu MB free of %lu MB capacity\r\n",
                 free_clust * fs->csize >> 11,
                 (fs->n_fatent - 2) * fs->csize >> 11);
    } else {
...
static void ftp_cmd_stat(ftp_data_t *ftp) {
    ...
    // print status
    ftp_send(ftp,
             "221 FTP Server status: you will be disconnected after %d minutes "
             "of inactivity\r\n",
             FTP_TIME_OUT_S / 60);
}
...
void ftp_service(struct netconn *ctrlcn, ftp_data_t *ftp) {
    ...
    // bugfix which works around ports which are already in use (from a previous
    // connection)
    ftp->data_port_incremented =
        (ftp->data_port_incremented + 1) % PORT_INCREMENT_OFFSET;
